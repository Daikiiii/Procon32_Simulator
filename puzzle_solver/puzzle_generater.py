...
if __name__=='__main__':
    img_path="C:/Procon32_Simulator/puzzle_solver/puzzle/puzzle1.jpg"
    pieces=puzzle_generater(img_path,3,4,1)
    img=np.vstack(( np.hstack((pieces[0:4])),
                    np.hstack((pieces[4:8])),
                    np.hstack((pieces[8:12]))))
    print(img.shape)
    cv2.imwrite("C:/Procon32_Simulator/puzzle_solver/puzzle/puzzle4.jpg",cv2.cvtColor(img,cv2.COLOR_BGR2RGB))    
'''